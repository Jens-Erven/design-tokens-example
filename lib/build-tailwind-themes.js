  ...
  for (const [key, token] of Object.entries(tokens)) {
    if (token.$type === 'color') {
      ...
      if (key.includes('primary') || key.includes('secondary') || 
          key.includes('error') || key.includes('warning') || 
          key.includes('info') || key.includes('success')) {
      ...
      if (key.includes('text')) {
        ...
        colorMappings.push(`    --color-text-${textName}: var(--${tokenName});`);
      }
    }
  }
  ...
  for (const [key, token] of Object.entries(tokens)) {
    if (token.$type === 'dimension' || key.includes('spacing')) {
      ...
      if (key.includes('spacing')) {
        ...
        spacingMappings.push(`    --spacing-${spacingName}: var(--${tokenName});`);
      }
