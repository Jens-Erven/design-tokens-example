...
const createConfig = (themeName, mode, tokens, outputDir) => ({
  ...
  platforms: {
    ...
    scss: {
      ...
      basePxFontSize: 16,
      buildPath: `${outputDir}/${themeName}/${mode}/scss/`,
      files: [
        {
    ...
    js: {
      ...
      buildPath: `${outputDir}/${themeName}/${mode}/js/`,
      files: [
        {
...
const buildMultiThemes = async () => {
  try {
    ...
    const outputDir = './dist/figma-variables';
...
function generateIndexFile(themes) {
  ...
  const imports = themeNames.flatMap(themeName => [
    `import * as ${camelize(themeName)}Light from './${themeName}/light/js/tokens.js';`,
    `import * as ${camelize(themeName)}Dark from './${themeName}/dark/js/tokens.js';`
  ]).join('\n');
